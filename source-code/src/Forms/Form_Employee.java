 ...
 * @author ThanhNhan
 */
public class Form_Employee extends javax.swing.JPanel implements Form_Employee_AddItem.DataChangedListener, Form_Employee_UpdateItem.DataChangedListener {
    ...
    public Form_Employee() {
        ...
        for (int i = 0; i < tableEmployees.getColumnCount(); i++) {
            tableEmployees.getColumnModel().getColumn(i).setCellRenderer(renderer);
        }
    }

    public void loadEmployeeToTable() {
        DefaultTableModel model = (DefaultTableModel) tableEmployees.getModel();
        model.setRowCount(0);
        
        List<Models.Employee> employees = employeeRepo.getAllEmployees();

        for (Models.Employee c : employees) {
            model.addRow(new Object[]{
                c.getEmployeeID(), 
                c.getEmployeeName(), 
                c.getEmployeeGender(),
                c.getEmployeeDOB(),
                c.getEmployeeAddress(),
                c.getEmployeePhone(),
                c.getEmployeeDOW(),
                c.getEmployeeUsername(),
                c.getEmployeePassword(),
                c.getPositionID(),
                c.getPositionName()
            });
        }
    ...
    private void initComponents() {
        ...
        tableEmployees.setFont(new java.awt.Font("Tahoma", 0, 16)); // NOI18N
        tableEmployees.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã nhân viên", "Họ tên nhân viên", "Giới tính", "Ngày sinh", "Địa chỉ", "Số điện thoại", "Ngày vào làm", "Tên đăng nhập", "Mật khẩu", "Mã chức vụ", "Tên chức vụ"
            }
        ));
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        Form_Employee_AddItem dialog = new Form_Employee_AddItem((Frame)SwingUtilities.getWindowAncestor(this), true);
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        ...
        String employeeIDToDelete = (String) tableEmployees.getValueAt(selectedRow, 0);

        int confirmResult = JOptionPane.showConfirmDialog(this,
            "Bạn có chắc chắn muốn xóa nhân viên có mã nhân viên: " + employeeIDToDelete.trim() + "?",
            "Xác nhận xóa",
            JOptionPane.YES_NO_OPTION);

        if (confirmResult == JOptionPane.YES_OPTION) {
            if (employeeRepo.deleteEmployee(employeeIDToDelete)) {
    ...
    private void btnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditActionPerformed
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một nhân viên để sửa thông tin.");
            return;
        }

        String employeeID = (String) tableEmployees.getValueAt(selectedRow, 0);
        String employeeName = (String) tableEmployees.getValueAt(selectedRow, 1);
        String gender = (String) tableEmployees.getValueAt(selectedRow, 2);
        java.util.Date dob = (java.util.Date) tableEmployees.getValueAt(selectedRow, 3);
        String address = (String) tableEmployees.getValueAt(selectedRow, 4);
        String phone = (String) tableEmployees.getValueAt(selectedRow, 5);
        java.util.Date dow = (java.util.Date) tableEmployees.getValueAt(selectedRow, 6);
        String username = (String) tableEmployees.getValueAt(selectedRow, 7);
        String password = (String) tableEmployees.getValueAt(selectedRow, 8);
        String positionID = (String) tableEmployees.getValueAt(selectedRow, 9);
        String positionName = (String) tableEmployees.getValueAt(selectedRow, 10);

        // Tạo một instance của Form_Employee_UpdateItem
        Form_Employee_UpdateItem updateItemDialog = new Form_Employee_UpdateItem((java.awt.Frame) SwingUtilities.getWindowAncestor(this), true);
        ...
        updateItemDialog.setEmployeeName(employeeName);
        updateItemDialog.setEmployeeGender(gender);
        updateItemDialog.setEmployeeDOB(new SimpleDateFormat("yyyy-MM-dd").format(dob));
        updateItemDialog.setEmployeeAddress(address);
        updateItemDialog.setEmployeePhone(phone);
        updateItemDialog.setEmployeeDOW(new SimpleDateFormat("yyyy-MM-dd").format(dow));
        updateItemDialog.setEmployeeUsername(username);
        updateItemDialog.setEmployeePassword(password);
        updateItemDialog.setEmployeePositionName(positionName);
        updateItemDialog.setEmployeePositionID(positionID); 
    ...
    private void btnInfoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInfoActionPerformed
        ...
        Form_Employee_DetailInfo detailInfoDialog = new Form_Employee_DetailInfo((java.awt.Frame) SwingUtilities.getWindowAncestor(this), true);
        ...
        detailInfoDialog.setEmployeeName(employeeName);
        detailInfoDialog.setEmployeeGender(gender);
        detailInfoDialog.setEmployeeDOB(new SimpleDateFormat("yyyy-MM-dd").format(dob));
        detailInfoDialog.setEmployeeAddress(address);
        detailInfoDialog.setEmployeePhone(phone);
        detailInfoDialog.setEmployeeDOW(new SimpleDateFormat("yyyy-MM-dd").format(dow));
        detailInfoDialog.setEmployeeUsername(username);
        detailInfoDialog.setEmployeePassword(password);
        detailInfoDialog.setEmployeePositionName(positionName);
        detailInfoDialog.setEmployeePositionID(positionID); 
    ...
    private void btnExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcelActionPerformed
        ...
        try {
            ...
            if (userSelection == JFileChooser.APPROVE_OPTION) {
                ...
                int columnCount = tableEmployees.getColumnCount();

                for (int i = 0; i < columnCount; i++) {
                    Cell cell = headerRow.createCell(i);
                    cell.setCellValue(tableEmployees.getColumnName(i));
                }
                ...
                for (int j = 0; j < rowCount; j++) {
                    ...
                    for (int k = 0; k < columnCount; k++) {
                        ...
                        Object value = tableEmployees.getValueAt(j, k);
